...
## Local Python check (non-Oryonix)

With network access and a valid key:

```bash
pip install -r requirements.txt
python -c "
from service.main import fetch_business_partners
rows = fetch_business_partners(service_url='https://sandbox.api.sap.com/s4hanacloud/sap/opu/odata/sap/API_BUSINESS_PARTNER', api_key='YOUR_KEY', top=3)
print(rows)
"
```
...
Set GitHub variables/secrets for `onix artifact build` / `upload` as in `.github/workflows/ci.yml`.
